...
public class RoomDAO implements DAOInterface<Room> {
	...
	public int insert(Room t) {
			...
			try {
				...
				String sql = "INSERT INTO room(rID,	roomType, floor, status, note) "
						+ "VALUES (?, ?, ?, ?, ?)" ;
	...
	public int update(Room t) {
		...
		try {
			...
			String sql = "UPDATE room "
					+ "SET roomType = ?, floor = ?, status = ?, note = ?"
					+ "WHERE rid = ? " ;
			PreparedStatement prst = conn.prepareStatement(sql);
			prst.setString(1, t.getRoomType());
			prst.setInt(2, t.getFloor());
			prst.setString(3, t.getStatus());
			prst.setString(4, t.getNote());
			prst.setString(5, t.getrID());
